...
import { LedgerEntry, ItemOption, LedgerFilters } from '../../Types/stock-ledger';
import AuthenticatedLayout from '../../Layouts/AuthenticatedLayout';
import { PageHeader, Breadcrumbs, Combobox, Label } from '../../Components/UI';
import { ComboboxOption } from '../../Components/UI/Combobox';
import { formatDate, formatNumber } from '../../Lib/formatters';
import { MONTH_OPTIONS_ALL, getYearOptions } from '../../Lib/constants';
...
export default function StockLedger({ entries, items, filters }: Props) {
    const itemOptions: ComboboxOption[] = [
        ...
        ...items.map(i => ({ value: i.id.toString(), label: i.item_code ? `${i.item_code} — ${i.name}` : i.name })),
    ];

    const selectedItem = items.find(i => i.id === filters.item_id);

    const handleFilter = (key: string, value: string) => {
        router.get('/reports/stock-ledger', { ...filters, [key]: value || undefined },
    ...
    return (
        <AuthenticatedLayout title="Kartu Mutasi Stok">
            ...
            <Breadcrumbs items={[{ label: 'Laporan', href: '/reports' }, { label: 'Kartu Mutasi Stok' }]} />
            <PageHeader title="Kartu Mutasi Stok" description="Riwayat pergerakan stok per barang" />
            <ReportTabs active="ledger" />
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-4">
                        <div className="flex flex-wrap items-center justify-between gap-4">
                            ...
                            <div className="flex gap-6 text-sm">
                                <div className="text-center"><p className="text-green-600 font-bold text-lg">{formatNumber(totalIn)}</p><p className="text-gray-400">Total Masuk</p></div>
                                <div className="text-center"><p className="text-red-600 font-bold text-lg">{formatNumber(totalOut)}</p><p className="text-gray-400">Total Keluar</p></div>
                                <div className="text-center"><p className="text-[#003366] font-bold text-lg">{formatNumber(lastBalance)}</p><p className="text-gray-400">Saldo Akhir</p></div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {entries.length === 0 ? (
                                        <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Tidak ada data mutasi untuk periode ini</td></tr>
                                        ...
                                        return (
                                            <tr key={entry.id} className="hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="px-4 py-2.5 text-right text-green-700 font-medium">{entry.qty_in > 0 ? `+${formatNumber(entry.qty_in)}` : '-'}</td>
                                                <td className="px-4 py-2.5 text-right text-red-700 font-medium">{entry.qty_out > 0 ? `-${formatNumber(entry.qty_out)}` : '-'}</td>
                                                <td className="px-4 py-2.5 text-right font-bold text-gray-900">{formatNumber(entry.balance)}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
